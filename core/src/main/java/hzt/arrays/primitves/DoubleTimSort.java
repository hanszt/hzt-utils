...
public final class DoubleTimSort {
    ...
    private DoubleTimSort(double[] array, DoubleComparator comparator) {
        this.array = array;
        this.comparator = comparator;
        int length = array.length;
        int tempLength = length < 512 ? (length >>> 1) : INITIAL_TMP_STORAGE_LENGTH;
        this.temporaryArray = new double[tempLength];
        this.tmpBase = 0;
    ...
    static void sort(double @NotNull [] array, int fromIndex, int toIndex, @NotNull DoubleComparator comparator) {
        ...
        if (nRemaining >= 2) {
            if (nRemaining < MIN_MERGE) {
                ...
                DoubleTimSort ts = new DoubleTimSort(array, comparator);
    ...
    private static int getLo(double[] a, int lo, int hi, DoubleComparator c, int nRemaining, DoubleTimSort ts, int minRun) {
        do {
    ...
    private static void binarySort(double[] array, int lo, int hi, int start, DoubleComparator c) {
        ...
        while (start < hi) {
            ...
            while (left < right) {
                n = (left + right) >>> 1;
                if (c.compareDouble(pivot, array[n]) < 0) {
                    right = n;
                } else {
                    left = n + 1;
                }
    ...
    private static int countRunAndMakeAscending(double[] a, int lo, int hi, DoubleComparator c) {
        ...
        if (runHi == hi) {
            ...
            if (c.compareDouble(a[runHi++], a[lo]) >= 0) {
                while (runHi < hi && c.compareDouble(a[runHi], a[runHi - 1]) >= 0) {
                    ++runHi;
                }
            } else {
                while (runHi < hi && c.compareDouble(a[runHi], a[runHi - 1]) < 0) {
    ...
    private static int gallopLeft(double key, double[] a, int base, int len, int hint, DoubleComparator c) {
        PreConditions.require(hint >= 0 && hint < len);

        int lastOfs = 0;
        int ofs = 1;
        int m;
        if (c.compareDouble(key, a[base + hint]) > 0) {
            m = len - hint;

            while (ofs < m && c.compareDouble(key, a[base + hint + ofs]) > 0) {
                lastOfs = ofs;
                ofs = (ofs << 1) + 1;
                if (ofs <= 0) {
                    ofs = m;
        ...
        while (lastOfs < ofs) {
            ...
            if (c.compareDouble(key, a[base + m]) > 0) {
                lastOfs = m + 1;
            } else {
                ofs = m;
            }
    ...
    private static int gallopRight(double key, double[] a, int base, int len, int hint, DoubleComparator c) {
        PreConditions.require(hint >= 0 && hint < len);
        int ofs = 1;
        int lastOfs = 0;
        int m;
    ...
    private void mergeLo(int base1, int len1, int base2, int len2) {
        ...
        double[] tmp = this.ensureCapacity(len1);
        ...
        if (len2 == 0) {
            ...
            DoubleComparator c = this.comparator;
            ...
            while (true) {
                ...
                while (len1 > 1 && len2 > 0) {
                    label145:
                    {
                        if (c.compareDouble(array[cursor2], tmp[cursor1]) < 0) {
    ...
    private void mergeHi(int base1, int len1, int base2, int len2) {
        ...
        double[] array = this.array;
        double[] tmp = this.ensureCapacity(len2);
        ...
        if (len1 == 0) {
            ...
            while (true) {
                ...
                while (len1 > 0 && len2 > 1) {
                    label145:
                    {
                        if (c.compareDouble(tmp[cursor2], array[cursor1]) < 0) {
                            array[dest--] = array[cursor1--];
                            ++count1;
                            count2 = 0;
                            --len1;
