...
public final class IntTimSort extends PrimitiveTimSort<int[], IntComparator> {
    ...
    private IntTimSort(int[] intArray, IntComparator intComparator) {
        ...
        this.tempArray = new int[getInitTempLength(intArray.length)];
    }
    ...
    int getRunLen(int[] array, int lo, int hi, IntComparator comparator, int nRemaining, int minRun) {
    ...
    void mergeAt(int i) {
        ...
        --stackSize;
        final int ofsRight = gallopRight(intArray[base2], intArray, base1, len1, 0, intComparator);
        ...
        final int ofsLeft = gallopLeft(intArray[base1 + len1 - 1], intArray, base2, len2, len2 - 1, intComparator);
    ...
    private static int gallopLeft(int key, int[] array, int base, int len, int hint, IntComparator comparator) {
    ...
    void mergeHi(int base1, int len1, int base2, int len2) {
        ...
        int[] array = intArray; // For performance
        int[] tmpArray = ensureCapacity(len2);
        final int tmpBase = 0;
        ...
        while (true) {
            ...
            do {
                ...
                if (comparator.compareInt(tmpArray[cursor2], array[cursor1]) < 0) {
                    array[dest--] = array[cursor1--];
                    count1++;
                    count2 = 0;
                    if (--len1 == 0) {
    ...
    private int[] ensureCapacity(int minCapacity) {
        if (tempArray.length < minCapacity) {
            final int newLength = calculateNewLength(minCapacity, intArray.length);
