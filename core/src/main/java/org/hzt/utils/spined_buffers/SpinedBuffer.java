...
public class SpinedBuffer<E>
        ...
        class Splitr implements Spliterator<E> {
            ...
            public void forEachRemaining(final Consumer<? super E> consumer) {
                ...
                if ((splSpineIndex < lastSpineIndex)
                    ...
                    final E[] chunk = (splSpineIndex == lastSpineIndex) ? splChunk : spine[lastSpineIndex];
            ...
            public Spliterator<E> trySplit() {
                if (splSpineIndex < lastSpineIndex) {
                    ...
                    final int t = (lastSpineElementFence - splElementIndex) / 2;
                    if (t == 0) {
                        return null;
                    } else {
                        final Spliterator<E> ret = Arrays.spliterator(splChunk, splElementIndex, splElementIndex + t);
                        splElementIndex += t;
                        return ret;
                    }
                } else {
    ...
    abstract static class OfPrimitive<E, T_ARR, T_CONS>
       ...
       protected T_ARR curChunk;
        ...
        public void copyInto(final T_ARR array, int offset) {
            final long finalOffset = offset + count();
        ...
        public T_ARR asPrimitiveArray() {
            final long size = count();
            if (size >= MAX_ARRAY_SIZE) {
                throw new IllegalArgumentException(BAD_SIZE);
            }
            final T_ARR result = newArray((int) size);
            copyInto(result, 0);
            return result;
        }
        ...
        abstract class BaseSpliterator<T_SPLITR extends Spliterator.OfPrimitive<E, T_CONS, T_SPLITR>>
            ...
            public void forEachRemaining(final T_CONS consumer) {
                ...
                if ((splSpineIndex < lastSpineIndex)
                    ...
                    // completed chunks, if any
                    for (int sp = splSpineIndex; sp < lastSpineIndex; sp++) {
                        final T_ARR chunk = spine[sp];
                        arrayForEach(chunk, i, arrayLength(chunk), consumer);
                        i = 0;
                    }
                    // last (or current uncompleted) chunk
                    final T_ARR chunk = (splSpineIndex == lastSpineIndex) ? splChunk : spine[lastSpineIndex];
            ...
            public T_SPLITR trySplit() {
                if (splSpineIndex < lastSpineIndex) {
                    // split just before last chunk (if it is full this means 50:50 split)
                    final T_SPLITR ret = newSpliterator(splSpineIndex, lastSpineIndex - 1,
                    ...
                    if (t == 0) {
                        ...
                        final T_SPLITR ret = arraySpliterator(splChunk, splElementIndex, t);
                        splElementIndex += t;
                        return ret;
                    }
                } else {
    ...
    public static class OfLong extends SpinedBuffer.OfPrimitive<Long, long[], LongConsumer>
        ...
        @Override
        public String toString() {
            final long[] array = asPrimitiveArray();
            if (array.length < 200) {
                return String.format(CHUNK_TO_STRING_FORMAT,
                        getClass().getSimpleName(), array.length,
                        spineIndex, Arrays.toString(array));
            } else {
                final long[] array2 = Arrays.copyOf(array, 200);
                return String.format(CHUNK_TO_STRING_FORMAT + "...",
                        getClass().getSimpleName(), array.length,
                        spineIndex, Arrays.toString(array2));
            }
    ...
    public static class OfDouble
        ...
        @Override
        public String toString() {
            final double[] array = asPrimitiveArray();
            if (array.length < 200) {
                return String.format(CHUNK_TO_STRING_FORMAT,
                        getClass().getSimpleName(), array.length,
                        spineIndex, Arrays.toString(array));
            } else {
                final double[] array2 = Arrays.copyOf(array, 200);
                return String.format(CHUNK_TO_STRING_FORMAT + "...",
                        getClass().getSimpleName(), array.length,
                        spineIndex, Arrays.toString(array2));
            }
